...
" **************** IMPORT DATA ********************"
path = "C:/Users/elcrio/Desktop"  # Directory to download dataset
...
class SGCNet(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = SGConv(in_channels= data.num_features, # Number of features
                   out_channels= 32, # Dimension of embedding
                   K = 3, cached =True)
        self.conv2 = SGConv(in_channels=32,out_channels=dataset.num_classes,K=3,cached=True)
    def forward(self):
        x = self.conv1(data.x,  data.edge_index) #Applying convolution to data
        X=F.relu(x)
        x = self.conv2(X, data.edge_index)  # Applying convolution to data
